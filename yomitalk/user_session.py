...
class UserSession:
    ...
    def update_audio_generation_state(self, browser_state: Optional[dict] = None, **kwargs) -> dict:
        """Update audio generation state in BrowserState.
        ...
        Returns:
            dict: Updated BrowserState (or None for backward compatibility)
        """
        # Backward compatibility: if no browser_state provided, just return None
        if browser_state is None:
            logger.debug("Legacy update_audio_generation_state call - ignoring")
            return {}

        for key, value in kwargs.items():
            if key in browser_state["audio_generation_state"]:
                browser_state["audio_generation_state"][key] = value
                logger.debug(f"Audio state updated - {key}: {value}")

        # Update last update time
        browser_state["audio_generation_state"]["last_update"] = time.time()

        return browser_state

    def reset_audio_generation_state(self, browser_state: Optional[dict] = None) -> dict:
        ...
        Returns:
            dict: Updated BrowserState (or empty dict for backward compatibility)
        """
        # Backward compatibility: if no browser_state provided, just return empty dict
        if browser_state is None:
            logger.debug("Legacy reset_audio_generation_state call - ignoring")
            return {}
    ...
    def is_audio_generation_active(self, browser_state: Optional[dict] = None) -> bool:
        ...
        return bool(audio_state.get("is_generating")) and audio_state.get("status") == "generating"
    ...
    def get_session_restoration_info(self) -> Dict[str, Any]:
        ...
        return {
            "session_id": self.session_id,
            "missing_api_keys": self.needs_api_key_restoration(),
            "current_api_type": (self.text_processor.current_api_type.value if self.text_processor.current_api_type else None),
        }
    ...
    def get_recovery_progress_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": "idle",
            "progress": 0.0,
            "streaming_parts_count": 0,
            "estimated_total_parts": 1,
            "final_audio_available": False,
            "generation_id": None,
            "start_time": None,
        }

    def _cleanup_directory(self, *args: Any, **kwargs: Any) -> int:
        """Temporary compatibility method for tests."""
    ...
    # Compatibility wrappers for old method signatures
    def update_audio_generation_state_legacy(self, **kwargs: Any) -> None:
        """Legacy compatibility wrapper for update_audio_generation_state."""
        _ = kwargs  # Unused parameter
        pass

    def get_audio_generation_status_legacy(self) -> Dict[str, Any]:
        """Legacy compatibility wrapper for get_audio_generation_status."""
        return self.audio_generation_state.copy()

    def has_generated_audio_legacy(self) -> bool:
        """Legacy compatibility wrapper for has_generated_audio."""
        return False

    def is_audio_generation_active_legacy(self) -> bool:
        """Legacy compatibility wrapper for is_audio_generation_active."""
        return False

    def reset_audio_generation_state_legacy(self) -> None:
        """Legacy compatibility wrapper for reset_audio_generation_state."""
        pass
