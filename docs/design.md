...
- **ブラウザ状態復元**: localStorage統合による完全なセッション復元
  - ブラウザリフレッシュ時の設定・状態自動復元
  - 音声生成進捗の継続・再開機能
  - 接続断後の完全状態復旧
...
- **ビジュアル表示**:
  ...
  - 時間情報（経過時間、推定残り時間）
- **状態復元**: 接続断後の進捗状態復元
- **ブラウザ統合**: localStorage連携による進捗情報永続化
...
- **ブラウザ状態管理**: localStorage統合による永続的な状態保存
- **自動保存**: 設定変更時の自動セッション保存
- **復元処理**: アプリケーション開始時の既存セッション検出と復元
- **エラーハンドリング**: セッション復元失敗時の新規セッション作成
- **音声生成状態**: 詳細な進捗状態管理（パーツ数、時間、ファイルパス）
- **クロスプラットフォーム**: サーバーサイドとブラウザサイドの状態同期

#### Gradio状態管理の使い分け

**`gr.State`（セッション状態）**:
- セッション内で関数呼び出し間でのデータ永続化
- チャット履歴、内部処理状態など、UIに直接表示されないデータ
- サーバーRAM内での一時保存（ページリフレッシュで消失）
- 自動クリーンアップ機能（60分後）

**`gr.BrowserState`（ブラウザ状態）**:
- ブラウザ再起動後も保持したい設定やユーザー選択
- APIキー、ユーザー設定、保存された会話など
- localStorage暗号化保存（ページリフレッシュ後も保持）
- アプリ再起動時は消失

**UIコンポーネント（`gr.Textbox`など）**:
- ユーザーが直接表示・編集する一時的な値
- フォーム入力、即座に処理される一過性のデータ
- 関数の入出力として直接使用される値
...
- **ブラウザ状態復元**: localStorage統合による完全なクロスセッション状態管理
- **接続復旧**: ブラウザリフレッシュ後の完全状態復元
- **セキュリティ**: APIキー除外、機密情報保護
- **自動クリーンアップ**: 古いセッションの定期削除
- **パフォーマンス最適化**: セッション初期化の簡素化と最適なタイムアウト設定
...
### ブランチ戦略
- **フィーチャーブランチ**: `feature/機能名` 形式で新機能開発
- **バグ修正ブランチ**: `bugfix/修正内容` 形式でバグ修正
- **リファクタリングブランチ**: `refactor/対象範囲` 形式でコード改善
- **雑務ブランチ**: `chore/作業内容` 形式で依存関係更新やツール設定変更
- **メインブランチ**: `main` - 安定版コードベース（全ての変更はPRを通じてmainにマージ）

### コード品質
- **PEP 8準拠**: black、isort、flake8による自動フォーマット
- **型安全**: mypy厳格型チェック、型ヒント必須
- **テスト駆動**: TDD実践、統合前テスト必須
- **プルリクエスト開発**: フィーチャーブランチでの開発（レビューは不要であり, 変更記録を分かりやすく残す目的）
...
- **統合**: API連携、音声生成パイプライン
- **進捗表示**: 状態管理、UI更新、復元機能
- **パフォーマンス最適化**: タイムアウト値最適化とテスト速度改善
- **ブラウザ状態テスト**: localStorage連携とセッション復元の検証
...
- **セッション**: 必要時のみ状態保存・読込
- **ファイル**: 効率的な一時ファイル管理
- **タイムアウト最適化**: セッション初期化および音声生成のタイムアウト値最適化
- **ブラウザ状態同期**: localStorageとサーバーサイド状態の効率的同期
- **シンプル化**: セッション管理ロジックの簡素化とメモリ使用量減少
